...
class ModelMemory:
    ...
    Trace mode: FULL by design — this organ's deliverable includes the
    braid word itself, so per-op materialization is the product, not tax
    (see RUNMODEL_USAGE.md failure mode 7 for the measured observer law).
    """
    ...
    def format(self):
        ...
        or 'llama2c' once chatmodel — that handler's registrar — has been
        imported). All window access routes through this detection."""
    ...
    def toc(self):
        ...
        if self._toc is None:
            self._toc = registry.open_with(self.mem, self.format())
        return self._toc
    ...
    def tensor_bytes(self, name):
        ...
        return registry.open_with(self.mem, self.format(), tensor=name)
