"""
grade_compose.py — composition report over gradebook.txt: which taught
lines the student's answers actually composed from (receipts, not vibes).

    python3 grade_compose.py          # reads gradebook.txt in this folder
"""
import json, sys
def main():
    import os
    if not (os.path.exists('gradebook.txt') and os.path.exists('answers.log')):
        print(__doc__); print('(needs gradebook.txt AND answers.log here — run from hcl-ai/ after a tutor session)'); return
