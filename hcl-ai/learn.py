...
class Trainer:
    ...
    def _forward_tape(self, toks):
        ...
        for l in range(L):
            ...
            xs = [e.t.vadd(x, p) for x, p in zip(xs, proj)]
            ...
            xs = [e.t.vadd(x, p) for x, p in zip(xs, mlp)]
            self._tick()
    ...
    def _rms_f(self, x, w):
        ...
        e = self.e
        ss = e.dot(x, x)                       # fused COMP-loop of AMP_MOD
        ss = e.add(e.mul(ss, self.m.inv_dim), self.m.EPS)
        inv = e.div(self.ONE, e.sqrt(ss))
        y = e.t.vscale(e.t.vmul(w, x), inv)    # fused, counted
        return y, (list(x), inv)
