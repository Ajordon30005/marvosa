# HCL Engine — Complete Implementation Reference
...
All arithmetic uses Python's arbitrary-precision `int`. No floats anywhere.
No overflow. No rounding. Every value is exact.

---

## Fixed-Point Integer Primitives

These are the ONLY arithmetic operations used. They operate on integers
where all values represent x = X/SCALE.
...
**CRITICAL on _fixed_sqrt:** The target is X*SCALE, NOT X. The Newton step
is `target // Rg`, NOT `_fixed_div(X, Rg)`. Using _fixed_div gives wrong
results because the initial guess is not correctly scaled.
...
def _sin_fp(X_rad: int, terms: int = 40) -> int:
    ...
    for k in range(terms):
        fact = SCALE
        for j in range(1, 2*k+2): fact = _fixed_mul(fact, j * SCALE)
...
def _cos_fp(X_rad: int, terms: int = 40) -> int:
    ...
    xpow   = _fixed_mul(X_rad, X_rad)
    for k in range(1, terms):
        fact = SCALE
        for j in range(1, 2*k+1): fact = _fixed_mul(fact, j * SCALE)
        term = _fixed_div(xpow, fact)
        result += term if k % 2 == 0 else -term
        xpow = _fixed_mul(xpow, _fixed_mul(X_rad, X_rad))
        if abs(term) < TINY: break
    return result
...
class HCLTranscriber:
    def __init__(self): self.braid_log = []
    def _log(self, op, r): self.braid_log.append({'op': op, 'r': r})
    ...
    def add(self, X, Y):
        r = HCL.COMP(FBit.from_scalar(X), FBit.from_scalar(Y))
        self._log('COMP[+]', r); return r.to_scalar()

    def sub(self, X, Y):
        r = HCL.COMP(FBit.from_scalar(X),
                     HCL.SHIFT(FBit.from_scalar(Y), -SCALE))
        self._log('COMP[-]', r); return r.to_scalar()

    def mul(self, X, Y):
        r = HCL.AMP_MOD(FBit.from_scalar(X), FBit.from_scalar(Y))
        self._log('AMP_MOD[×]', r); return r.to_scalar()

    def div(self, X, Y):
        r = HCL.AMP_MOD(FBit.from_scalar(X), HCL.INV(FBit.from_scalar(Y)))
        self._log('AMP_MOD[÷]', r); return r.to_scalar()
    ...
    def derivative(self, f, X: int) -> int:
        import math as _m
        dt     = _m.isqrt(ETA * LAMBDA**5 // SCALE**4) or (SCALE // 100000)
        # cleaner: dt = ETA * LAMBDA^5 / SCALE^4 in integer
...
```python
# Setup (run once)
fp  = HCLTranscriber.to_fp   # converts Python number to fixed-point
rfp = HCLTranscriber.from_fp  # converts fixed-point to decimal string
eq  = HCLEquation()
t   = HCLTranscriber()

# Arithmetic
t.add(fp(3), fp(4))           # 7 (via COMP)
t.mul(fp(7), fp(8))           # 56 (via AMP_MOD)
t.div(fp(22), fp(7))          # 22/7 exactly (via INV + AMP_MOD)
t.pow_int(fp(2), 10)          # 1024 (10 × AMP_MOD)
t.sqrt(fp(2))                 # √2 (via FISSION)

# Transcendental
t.exp(fp(1))                  # e (via MOBIUS_GROWTH)
t.exp(fp(-1))                 # 1/e
t.sin(SCALE//6)               # sin(π/6) = 0.5  [SCALE//6 = phase_frac for π/6]
t.cos(SCALE//4)               # cos(π/2) = 0

# Equation engine
result = eq.solve("E = m * c^2", m=1, c=3)
result = eq.solve("psi = exp(-x^2 / 2)", x=1)
result = eq.solve("gamma = 1 / sqrt(1 - (v/c)^2)", v=3, c=5)

# Phase fraction reference table
SCALE//2   = π      (180°, half loop)
SCALE//3   = 2π/3   (120°)
SCALE//4   = π/2    (90°)
SCALE//6   = π/3    (60°)
SCALE//8   = π/4    (45°)
...
1. **Never use `math.pi` or `numpy.pi`** — use `PI_INT` (the integer)
2. **Never pass radian floats to sin/cos** — convert to phase_frac first:
   `phase_frac = _fixed_div(angle_radians_fp, 2*PI_INT) % SCALE`
3. **Never use `_fixed_div(X, Xg)` in sqrt** — use `target // Rg` in Newton
4. **Never set MOBIUS_GROWTH output phase from input sign** — phase_out = 0 always
5. **Never feed pow_int output back into mul without checking it's fixed-point**
   — all operations return fixed-point integers; they chain correctly
6. **Never use Python float for intermediate steps** — everything stays integer
