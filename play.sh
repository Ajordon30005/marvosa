#!/usr/bin/env bash
# play.sh — MST door: chat with a REAL trained AI model running entirely on the HCL
...
# pass executed as pure-integer HCL primitives (~0.12s/token, ~8.7 tok/s) — verified token-exact against
